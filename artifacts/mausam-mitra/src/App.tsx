...
async function fetchWeather(location: LocationPoint, signal: AbortSignal): Promise<WeatherSnapshot> {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${location.latitude}&longitude=${location.longitude}&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,wind_speed_10m,precipitation,rain&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,rain_sum,uv_index_max,wind_speed_10m_max&forecast_days=7&timezone=auto`;
...
function useWeather(location: LocationPoint) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    fetchWeather(location, controller.signal).then(setData).catch((err: Error) => {
      if (err.name !== 'AbortError') setError(err.message);
    }).finally(() => setLoading(false));
    return () => controller.abort();
  }, [location.latitude, location.longitude, reload]);
  return { data, loading, error, refresh: () => setReload((value) => value + 1) };
}
...
function AdvisoryPage({ location, unit, language }: { location: LocationPoint; unit: Unit; language: Language }) {
  ...
  const advisories = useMemo(() => {
    ...
    if (today.rainChance >= 70) items.push({ level: 'WATCH', title: 'Rain may disrupt movement', detail: `${today.rainChance}% chance of rain today with up to ${today.rain.toFixed(1)} mm expected. Low-lying roads can change quickly.`, action: 'Leave a little earlier; protect tools, documents and electronics.', Icon: CloudRain });
    if (today.wind >= 35) items.push({ level: 'WATCH', title: 'Gusty winds possible', detail: `Wind may reach ${Math.round(today.wind)} km/h. Loose structures and exposed branches need attention.`, action: 'Secure light objects and avoid standing beneath old trees.', Icon: Wind });
    if (today.max >= 38) items.push({ level: 'CAUTION', title: 'Heat stress window', detail: `The afternoon high may reach ${formatTemp(today.max, unit)}. Heat load will be highest in the early afternoon.`, action: 'Shift heavy work to morning; drink water before you feel thirsty.', Icon: Sun });
